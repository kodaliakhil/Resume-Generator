...
export function ResumeForm({
  ...
  const onDownload = async () => {
    ...
    if (!ok) {
      show({ variant: "error", title: "Validation failed", message: "Fix errors before download." });
      return;
    }
    try {
    ...
    <form onSubmit={handleSubmit(onValidate)} noValidate className="space-y-6">
      ...
      <section className="card space-y-4">
        ...
        <TemplatePicker
          ...
          onChange={(tid) => setValue("templateId", tid, { shouldValidate: true, shouldDirty: true })}
          error={templateError}
        />
        ...
        <ThemeControls
          ...
          errors={{
            accentColor: errors.theme?.accentColor ? String(errors.theme.accentColor.message) : undefined
          }}
        />
      </section>
      ...
      <section className="card space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <input id="profile-phone" {...register("profile.phone")} className="w-full rounded-lg border border-white/10 bg-white/5 p-2" />
          </div>
          <div className="md:col-span-2">
            <label htmlFor="profile-location" className="mb-1 block text-sm font-medium">
              Location (optional)
            </label>
            <input id="profile-location" {...register("profile.location")} className="w-full rounded-lg border border-white/10 bg-white/5 p-2" />
          </div>
          <div className="md:col-span-2">
            <label htmlFor="profile-summary" className="mb-1 block text-sm font-medium">
              Summary (optional)
            </label>
            <textarea id="profile-summary" rows={4} {...register("profile.summary")} className="w-full rounded-lg border border-white/10 bg-white/5 p-2" />
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <Button type="button" variant="secondary" onClick={() => linkArray.append({ label: "", url: "" })}>
              + Add Link
            </Button>
          </div>
          {linkArray.fields.length === 0 && <p className="text-sm text-white/60">No links added yet.</p>}
              ...
              <div key={f.id} className="grid gap-3 md:grid-cols-12">
                <div className="md:col-span-4">
                  ...
                  {errors.profile?.links?.[idx]?.label?.message && (
                    <p className="mt-1 text-sm text-red-300">{String(errors.profile.links[idx]?.label?.message)}</p>
                  )}
                </div>
                <div className="md:col-span-7">
                  ...
                  {errors.profile?.links?.[idx]?.url?.message && (
                    <p className="mt-1 text-sm text-red-300">{String(errors.profile.links[idx]?.url?.message)}</p>
                  )}
                </div>
                <div className="md:col-span-1 flex items-end">
                  <Button type="button" variant="ghost" onClick={() => linkArray.remove(idx)} aria-label={`Remove link ${idx + 1}`} title="Remove">
                    ✕
                  </Button>
                </div>
              </div>
        ...
        {expArray.fields.map((f, idx) => (
          <div key={f.id} className="rounded-xl border border-white/10 p-3">
            <div className="grid gap-3 md:grid-cols-12">
              <div className="md:col-span-4">
                <label htmlFor={`experience-${idx}-company`} className="mb-1 block text-sm font-medium">
                  Company
                </label>
                <input
              ...
              <div className="md:col-span-4">
                <label htmlFor={`experience-${idx}-role`} className="mb-1 block text-sm font-medium">
                  Role
                </label>
                <input
              ...
              <div className="md:col-span-4">
                <label htmlFor={`experience-${idx}-location`} className="mb-1 block text-sm font-medium">
                  Location (optional)
                </label>
                <input id={`experience-${idx}-location`} {...register(`experience.${idx}.location` as const)} className="w-full rounded-lg border border-white/10 bg-white/5 p-2" />
              </div>
              <div className="md:col-span-3">
                <label htmlFor={`experience-${idx}-startDate`} className="mb-1 block text-sm font-medium">
                  Start (YYYY-MM)
                </label>
                <input
              ...
              <div className="md:col-span-3">
                <label htmlFor={`experience-${idx}-endDate`} className="mb-1 block text-sm font-medium">
              ...
              <label className="md:col-span-4 mt-7 flex items-center gap-2 text-sm">
                <input type="checkbox" {...register(`experience.${idx}.current` as const)} className="h-4 w-4" />
                Current role
              </label>
              <div className="md:col-span-2 mt-7 flex items-center justify-end">
                <Button type="button" variant="ghost" onClick={() => expArray.remove(idx)} aria-label={`Remove experience ${idx + 1}`} title="Remove">
                  ✕
                </Button>
              </div>
            </div>
          ...
          <div key={f.id} className="rounded-xl border border-white/10 p-3">
            <div className="grid gap-3 md:grid-cols-12">
              <div className="md:col-span-5">
                <label htmlFor={`education-${idx}-school`} className="mb-1 block text-sm font-medium">
                  School
                </label>
                <input
              ...
              <div className="md:col-span-5">
                <label htmlFor={`education-${idx}-degree`} className="mb-1 block text-sm font-medium">
                  Degree
                </label>
                <input
              ...
              <div className="md:col-span-2">
                ...
                <input id={`education-${idx}-gpa`} placeholder="8.5" {...register(`education.${idx}.gpa` as const)} className="w-full rounded-lg border border-white/10 bg-white/5 p-2" />
              </div>
              <div className="md:col-span-3">
                <label htmlFor={`education-${idx}-startDate`} className="mb-1 block text-sm font-medium">
                  Start (YYYY-MM)
                </label>
                <input
              ...
              <div className="md:col-span-3">
                <label htmlFor={`education-${idx}-endDate`} className="mb-1 block text-sm font-medium">
                  End (YYYY-MM)
                </label>
                <input
              ...
              <div className="md:col-span-2 mt-7 flex items-center justify-end">
                <Button type="button" variant="ghost" onClick={() => eduArray.remove(idx)} aria-label={`Remove education ${idx + 1}`} title="Remove">
                  ✕
                </Button>
              </div>
            </div>
